...
{
	bool ret = true;
	p2SString tmp("%s%s", folder.GetString(), map.GetString());

	char* buffer;
	int size = App->fs->LoadFile(tmp.GetString(), &buffer);
	pugi::xml_parse_result result = map_file.load_buffer(buffer, size);
	RELEASE(buffer);

	if(result == NULL)
	{
		LOG("Could not load map xml file %s. pugi error: %s", map.GetString(), result.description());
		ret = false;
	}
	...
	//LOGs to be sure everything is like it's supposed to be
	if(ret == true)
	{
		LOG("Successfully parsed map XML file: %s", map.GetString());
